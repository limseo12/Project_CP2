...
def comparaion():
    ...
    profile_photo_embedding_dict = get_face_embedding_dict(profile_path,"r") # 파일 이름과 변환된 백터 딕셔너리
    now_photo_embedding_dict = get_face_embedding_dict(now_face_path,"r")
...
# def comparaion():
#     profile_dir = 'images/color_now_face/' #현재 프로필 디렉토리
#     profile_name = os.listdir(profile_dir) #현재 프로필 이름

#     img_list = glob.glob('images/color_faces/*.jpg') #비교하는 사진 리스트
#     unrecog = 'images/unrecognized/' #얼굴탐지 못한 사진이 갈 주소
#     save = 'images/crop_faces/' #얼굴탐지 된 사진이 갈 주소
#     #모델변경 가능
...
#             resp.append(DeepFace.verify(img1_path=profile_dir+profile_name[0], img2_path=img, model_name=models[6], distance_metric=metrics[0]))
